...
namespace factory
{
    ...
    extern std::shared_ptr<FaceMesh> make_terrain(const HeightMap& hm,
                                                  float latScale=1.0f,
                                                  float texScale=1.0f);
    extern std::shared_ptr<TriangularMesh> make_terrain_tri_mesh(const HeightMap& hm,
                                                                 float latScale=1.0f,
                                                                 float texScale=1.0f);
    extern std::shared_ptr<FaceMesh>       make_uv_sphere(uint32_t nRings,
                                                          uint32_t nRingPoints);
    extern std::shared_ptr<TriangularMesh> make_ico_sphere(uint32_t refine=1,
                                                           bool finalize=true);
    extern std::shared_ptr<TriangularMesh> make_icosahedron(bool finalize=true);
    ...
    // Helpers
    extern void skin_spline(std::shared_ptr<FaceMesh> pmesh,
                            const math::CSplineCatmullV3& spline,
                            uint32_t nRings,
                            uint32_t nRingPoints,
                            float radius_0=0.1f,
                            float radius_exponent=1.0f);
    extern void subdivide_mesh(std::shared_ptr<TriangularMesh> pmesh);
}
